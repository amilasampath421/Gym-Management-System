...
<a href="profile.php?user_id=<?php echo $user_id?>" class="<?php if($page == ''): echo 'active';endif; ?>">Profile</a>
      ...
      <tr>
        <td >
			      <h1 id="llogin">Booking</h1>
        </td>
      </tr>
      <tr>
        <td>
        <input type="text" id="text" name="user_id" value="<?php echo $_GET['user_id'];?>"><br><br>
        </td>
      </tr>
      <tr>
        <td>
        <input type="text" readonly id="text" name="tool"value="<?php echo $_GET['tool'];?>"><br><br>
        </td>
      </tr>
      <tr>
        <td>
        <input type="time" id="text"name="time"><br><br>
        </td>
      </tr>
      <tr>
        <td>
        <input type="date" id="text"name="date"><br><br>
        </td>
      </tr>
      
      <tr>
        <td>
        <input id="button" type="submit" value="Book"name="book">
    </td>
      </table>
	</form>
	</div>
	<?php 
        if(isset($_POST['book'])){
            ...
            if($count==1){
               echo "<p class='error'>Already Booked</p>";
            }
           else{
                if(!empty($time) && !empty($date)){
                    $sql2="insert into booking(user_id,booking_item,time,date) value('$user_id','$tool','$time','$date');";
                    $res2=mysqli_query($con,$sql2);

                    //header("Location: booking.php");
                    echo "<p class='success'> Booked</p>";
                }
                else{
                    echo "<p class='error'>Enter the Data</p>";
                }
            }
       }
    ?>
